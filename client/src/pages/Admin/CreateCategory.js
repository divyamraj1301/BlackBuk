...
const CreateCategory = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.post(`${ServerAPI}/api/v1/category/create-category`, {
        name,
      });
      if (data?.success) {
      ...
      console.log(error);
      message.error("Something went wrong with input form");
    }
  };
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `${ServerAPI}/api/v1/category/update-category/${selected._id}`,
        { name: updatedName }
      );
      if (data.success) {
  ...
  const handleDelete = async (pId) => {
    try {
      const { data } = await axios.delete(
        `${ServerAPI}/api/v1/category/delete-category/${pId}`
      );
      if (data.success) {
